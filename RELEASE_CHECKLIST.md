...
| Sign in with Apple — App ID capability                 | REQUIRES MY MANUAL CONFIGURATION    | EAS enables it on the App ID during the credentials step; entitlement already in the build config                                                                                                                                           |
| Xcode 26 / iOS 26 toolchain on a production build      | REQUIRES TESTFLIGHT / APPLE SANDBOX | Confirmed when the first signed production build runs                                                                                                                                                                                       |
| Apple distribution certificate / provisioning          | REQUIRES MY MANUAL CONFIGURATION    | Membership active since 2026-09-13. Interactive: `cd apps/mobile && npx eas-cli credentials --platform ios`                                                                                                                                 |
| App Store Connect app record, subscription products    | REQUIRES MY MANUAL CONFIGURATION    | `docs/release/revenuecat-setup.md` App Store Connect steps                                                                                                                                                                                  |
| Production Supabase project                            | DONE                                | `juhkjelqfrbbhgzmedgs`: migrations 5/5, catalogue seeded once, 4 functions, config pushed, smoke 45/45, advisors clean (2026-09-13)                                                                                                         |
| Apple provider enabled on production Supabase          | DONE                                | Pushed via `[remotes.production]`; verify in the dashboard before the first real sign-in                                                                                                                                                    |
| RevenueCat project, offering, keys, webhook            | REQUIRES MY MANUAL CONFIGURATION    | Secrets exist on production but `REVENUECAT_SECRET_API_KEY` is malformed — re-set it bare; then the public key via `eas env:set`; `docs/release/revenuecat-setup.md`                                                                        |
| EAS `production` env → production Supabase             | DONE                                | Repointed 2026-09-13; `preview` stays on staging                                                                                                                                                                                            |
...
### Code-side release audit (2026-09-13, after the production Supabase step)

A fresh read of the release-critical surfaces found six defects fixable locally; all are fixed and tested.

| Finding                                                                                                                                                                                                                                         | Fix                                                                                                                                              | Class |
| ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------ | ----- |
| "Already owned" moved the purchase flow to `verifying`, which only re-read `entitlements` — nothing re-submitted the receipt, so a user whose Apple ID already held the subscription saw "bringing it back now" and then a verification failure | `entitlement-store.buy` runs a restore before the refresh for that one outcome (`entitlement-store.test.ts`, 3 cases)                            | DONE  |
| A merge conflict (`409`) or a failed merge left the device signed in as the account while every local store still belonged to the guest — the next launch cleared the dog and routed into onboarding for an account that already had one        | The guest session is put back (`resumeSession`), so the message "nothing changed" is true (`account-screen.test.tsx`)                            | DONE  |
| A stored session that failed to restore (revoked refresh token) was treated as "no session" and a new guest was created silently — the silent logout AUTH.md forbids                                                                            | `ensureAnonymousSession` reports the session unavailable instead; the account screen then signs in without a merge to recover the account's data | DONE  |
| iPad multitasking was enabled (`UIRequiresFullScreen=false`, all four iPad orientations) for a portrait-designed app no one has run in split view                                                                                               | `ios.requireFullScreen: true` — still offered on iPad, full-screen and portrait. Revisit only after an iPad layout pass                          | DONE  |
| No `CFBundleLocalizations`, so iOS offered no per-app language and the store would list English only, despite shipping Hebrew                                                                                                                   | Declared `en`, `he` (`CFBundleDevelopmentRegion` `en`)                                                                                           | DONE  |
| No status message anywhere was announced to VoiceOver/TalkBack (no live regions, no announcements): a failed purchase, an empty restore, a failed sign-in or deletion were silent to a screen-reader user still focused on the button           | `useAnnounce` on the paywall, billing section, account and deletion screens                                                                      | DONE  |

A seventh finding surfaced on production once the RevenueCat secrets existed: the first request to RevenueCat
threw (the secret's value is not a valid header value) and, because that path had never run anywhere, the
functions crashed to a raw runtime `500` instead of their JSON envelope. Every RevenueCat call is now wrapped and
answers `502 { providerDetail }`; the secret itself must be re-set by the account owner (`docs/release/revenuecat-setup.md`).

Also: Apple credential revocation now signs the device out (`installAppleRevocationHandler`); token refresh
follows the app's foreground state as Supabase recommends; the Apple button renders only on iOS; "cancels
anytime" → "cancel anytime". The real revocation event, VoiceOver announcements on a device and the iPad
full-screen layout are **PENDING EXTERNAL VALIDATION**.

**TRUE CODE BLOCKER: none.** Every remaining item is manual configuration, a device or store environment, or a
decision. The Apple membership is active (2026-09-13); nothing has been signed or submitted.
