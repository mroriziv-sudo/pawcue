...
- **Billing**: free user, trial, monthly, annual, expiration, cancellation, grace period, restore, refund, and
  **webhook replay idempotency** (the same store notification delivered twice must not double-apply).
  ...
  configuration file (product IDs and test-only settings; the prices in it are placeholders, not the store's).
  It works only when Xcode launches the app with it attached: `expo prebuild` regenerates `apps/mobile/ios`
  without any of this, so after a prebuild add a file reference to `../PawCue.storekit` in the project
  navigator and select it under Product → Scheme → Edit Scheme → Run → Options → StoreKit Configuration (the
  shared scheme then carries `StoreKitConfigurationFileReference identifier="../../PawCue.storekit"`). Launching
  with `simctl` or an EAS build ignores it, and storekitd asks the real store. The 2026-09-17 acceptance pass did
  exactly this, scripted through Xcode, and the paywall rendered both tiles from the file.

  `tools/ios-acceptance` holds a throwaway Xcode project (generated by `xcodegen` from `project.yml`) whose
  UI-test bundle attaches to whatever PawCue build is installed on the booted simulator and prints the
  accessibility tree — labels, values, selection — for the tab bar, Today, Train, a session and the Settings
  switch. That tree is what VoiceOver speaks, which the simulator cannot otherwise show. `tools/ios-acceptance/run.sh`
  runs it; the `A11Y` lines in its output are the evidence.
